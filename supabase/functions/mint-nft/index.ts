// Supabase Edge Function – Solana NFT Mint (Metaplex v3, FIXED)
...
serve(async (req) => {
  ...
  try {
    ...
    } = await import("npm:@metaplex-foundation/mpl-token-metadata@3.2.1");
    ...
    // ---------------- Wallet ----------------
    const payerSecret = base58Decode(
      Deno.env.get("SOLANA_PAYER_SECRET_KEY")!
    );
    ...
    return new Response(
      JSON.stringify({ success: true, mint: mint.toBase58(), sig }),
      {
